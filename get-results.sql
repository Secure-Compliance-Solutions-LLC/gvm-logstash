select r.host                                                                               as "ip",
       rhd_hostname.value                                                                   as "hostname",
       ...
       r.severity                                                                           as "cvss",
       (case
            when r.severity <= 0::real then 'Log'
            when r.severity >= 0.1::real and r.severity <= 3.9::real
                then 'Low'
            when r.severity >= 4.0::real and r.severity <= 6.9::real
                then 'Medium'
            when r.severity >= 7.0::real and r.severity <= 10.0::real
                then 'High' end)                                                            as "severity",
       (case
            when r.severity <= 0::real
                then 1
            else 0 end)                                                                     as "log",
       (case
            when r.severity >= 0.1::real and r.severity <= 3.9::real
       ...
       (case
            when severity >= 4.0::real and severity <= 6.9::real
                then 1
            else 0 end)                                                                     as "medium",
       (case
            when severity >= 7.0::real and severity <= 10.0::real
                then 1
            else 0 end)                                                                     as "high",
       r.qod                                                                                as "qod",
...
from results r
         ...
         left join (select *
                    ...
                    where id in (select max(id)
                                 from report_host_details
                                 where name = 'hostname'
                                 group by report_host, name)) rhd_hostname
                   on rh.id = rhd_hostname.report_host
         left join (select *
                    from report_host_details
                    where id in (select max(id)
                                 from report_host_details
                                 where name = 'best_os_txt'
                                 group by report_host, name)) rhd_os_txt
                   on rh.id = rhd_os_txt.report_host
         left join (select *
                    from report_host_details
                    where id in (select max(id)
                                 from report_host_details
                                 where name = 'best_os_cpe'
                                 group by report_host, name)) rhd_os_cpe
                   on rh.id = rhd_os_cpe.report_host
         left join (select *
                    from report_host_details
                    where id in (select max(id)
                                 from report_host_details
                                 ...
                                 group by report_host, name)) rhd_smb_auth
                   on rh.id = rhd_smb_auth.report_host
         left join (select *
                    from report_host_details
                    where id in (select max(id)
                                 from report_host_details
                                 where name = 'MAC'
                                 group by report_host, name)) rhd_mac
                   on rh.id = rhd_mac.report_host
         ...
         left join (select array_agg(ref_id) as ids, vt_oid from vt_refs where type = 'url' group by vt_oid) url
                   on n.oid = url.vt_oid
where re.scan_run_status = 1
  and r.qod > 70
  and to_timestamp(r.date) > now() - interval '2 months'
